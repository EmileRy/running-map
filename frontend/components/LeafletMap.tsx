...
export default function LeafletMap({ tracks, selectedDate }: {
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, {
      ...
      zoom: 6,
      zoomControl: true,
      preferCanvas: true, // Optimization: Use Canvas renderer for thousands of segments
    })
  ...
  useEffect(() => {
    ...
    for (const track of tracks) {
      ...
      const polyline = L.polyline(latlngs, {
        ...
        weight: 2,
        opacity: 0.65,
        smoothFactor: 2, // Increase smoothing to reduce rendering load
      }).addTo(map)
      allBounds.push(polyline.getBounds())
      polylinesRef.current.push({
        polyline,
        firstRunAt: track.firstRunAtMs, // Use pre-calculated timestamp (O(1))
      })
    }
