...
export default function LeafletMap({ tracks, selectedDate }: {
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, {
      ...
      zoom: 6,
      zoomControl: true,
      preferCanvas: true, // Optimisation : rendu sur un seul Canvas au lieu de multiples SVGs
    })
  ...
  useEffect(() => {
    ...
    for (const track of tracks) {
      ...
      // Calcul manuel des bounds en une seule passe pour éviter des milliers d'allocations d'objets LatLngBounds
      ...
      polylinesRef.current.push({
        polyline,
        firstRunAt: track.firstRunAtMs ?? (track.firstRunAt ? new Date(track.firstRunAt).getTime() : null),
      })
    }
