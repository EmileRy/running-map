...
export function MapLayout({ user, tracks, zones }: { user: User; tracks: Track[]; zones: Zone[] }) {
  ...
  useEffect(() => {
    // ⚡ Bolt: using setTimeout to avoid cascading render warnings from React 19 lint rules
    setTimeout(() => {
      setMounted(true)
    }, 0)
  }, [])
  ...
  useEffect(() => {
    const nextDate = maxDate ?? Date.now()
    setTimeout(() => {
  ...
  // ⚡ Bolt: memoizing slider ranges to satisfy purity and avoid re-calc on every render
  const { sliderMin, sliderMax } = useMemo(() => {
    if (!mounted) return { sliderMin: 0, sliderMax: 0 }
    // ⚡ Bolt: we use maxDate or 0 to stay pure. The actual slider range
    // will be stable once mounted. If maxDate is null, we use a fallback.
    const referenceDate = maxDate ?? 1712246400000

    return {
      sliderMin: minDate ?? (referenceDate - 5 * 365 * 24 * 60 * 60 * 1000),
      sliderMax: referenceDate
    }
  }, [minDate, maxDate, mounted])

  const { runCount, coveredLengthM } = useMemo(() => {
    let count = 0
    let length = 0
    for (const t of visibleTracks) {
      if (!t.firstRunAtMs || t.firstRunAtMs <= selectedDate) {
        count++
        length += t.lengthM
      }
    }
  ...
  useEffect(() => {
    // ⚡ Bolt: Defer state update to avoid cascading render warning
