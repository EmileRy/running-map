...
export function MapLayout({ user, tracks, zones, serverNow }: { user: User; tracks: Track[]; zones: Zone[]; serverNow: number }) {
  ...
  // On initialise avec maxDate si disponible pour éviter un useEffect immédiat
  ...
  // Sync selectedDate with maxDate changes (e.g. when changing zones)
  // On compare pour éviter de déclencher un re-render si la date est déjà correcte
  if (maxDate !== null && selectedDate === serverNow) {
    setSelectedDate(maxDate)
  }

  // Fallbacks using server-provided stable timestamp to ensure render purity
