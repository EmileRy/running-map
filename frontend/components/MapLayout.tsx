...
import { useIsMounted, useMountNow } from '@/lib/mount-now'
...
export function MapLayout({ user, tracks, zones }: { user: User; tracks: Track[]; zones: Zone[] }) {
  ...
  useEffect(() => {
    if (maxDate !== null) {
      // Use setTimeout to defer state update and satisfy react-hooks/set-state-in-effect
      const id = setTimeout(() => {
  ...
  // Fallbacks client-only (safe car utilisés seulement après montage via useMountNow)
